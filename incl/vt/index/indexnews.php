  ...
  static function getContent(){
    if(!is_null(Def\Cache_File::$cash)){
      ...
      if($index_news=='0'){
        ...
        $res=Def\SQListatic::arrSQL_('SELECT links,caption,index_text,data FROM zzz_news ORDER BY id DESC LIMIT 1');
        ...
        foreach($res as $k=>$v){
          ...
          $index_news.='<div class="ind_news"><div><h4 class="align-center">'.$v["caption"].'</h4>'.$v["index_text"].'</div><div class="cl"></div>
<div>&nbsp;&nbsp;&nbsp;<a href="/news/'.$v["links"].'">Узнать подробнее...</a><div class="ind_news_data align-center fr">'.$res_date.'</div></div></div><div class="cl"></div>';
        }
        $index_news.='</div>';
        echo $index_news;
