<?php
$ind_article=$Cash->IsSetCacheFile('common/article.html');
...
foreach($res as $k=>$v){
  if($v['img_i']!=''){$img='<a href="/статьи/'.$v['link'].'"><img class="fl five br" src="/img/article/dbpic.php?id='.$v['img_i'].'" alt="'.$v['img_alt_i'].'" title="'.$v['img_title_i'].' - узнать подробнее..."></a>';}else{$img='';}
...
}$Cash->StartCache();echo $ind_article;$Cash->StopCache('common/article.html');}else $ind_article='';}