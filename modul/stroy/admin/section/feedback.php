...
if(Validator::paternInt(Route::$uri_parts[2])){
    $DB=new SQLi();
        ...
        if(Post\Post::issetPostKey(['sms'])){
            ...
            if(Validator::paternInt($id)){
                if($DB->boolSQL('UPDATE feedback SET readed=1 WHERE id='.$DB->realEscapeStr($id)))
                    Opt::$main_content.='<div class="fon_c five_ b">Сообщение прочитано...</div>';
                else Opt::$main_content.='<div class="fon_c five_ b">Ошибка сообщения!!!</div>';
    ...
    $res=$DB->strSQL('SELECT readed,name,mail,theme,text,data FROM feedback WHERE id='.$DB->realEscapeStr(Route::$uri_parts[2]));
    if($res){
        ...
        if($res['readed']==''){
            Opt::$main_content.='<form class="form" method="post"><input type="hidden" name="sms" value="'.Route::$uri_parts[2].'"><input type="submit" value="Прочитано"></form>';
        }
